...
def get_halo_density_analytic(z, mdef='200m'):
    """Return spherical-overdensity halo density in kg/m^3."""
    if mdef == '200m':
        return 200.0 * Omega_m * rho_crit_z0_kgm3 * (1.0 + z)**3
    if mdef == '200c':
        ez2 = Omega_m * (1.0 + z)**3 + Omega_lambda
        return 200.0 * rho_crit_z0_kgm3 * ez2
    raise ValueError(f"Unknown mass definition: {mdef}")


def get_Rvir_analytic(M_vir_in_Msun, z, mdef='200m'):
    #M_vir in solar mass, return virial radius 
    #default uses Mean200/background-density definition
    #return R_vir in Mpc
    rho_halo = get_halo_density_analytic(z, mdef=mdef)
    R_vir = (3*M_vir_in_Msun*Msun/(4*np.pi*rho_halo))**(1/3)
    R_vir_Mpc = R_vir/Mpc
    return R_vir_Mpc

def Vel_Virial_analytic(M_vir_in_Msun, z, mdef='200m'):
    #M_vir in solar mass, return virial velocity in m/s
    #default uses Mean200/background-density definition
    #return V_vir in m/s
...
def inversefunc_Vel_Virial_analytic(V_vir, z, mdef='200m'):
...
def Temperature_Virial_analytic(M_vir_in_Msun,z,mean_molecular_weight=None, mdef='200m'):  #van den Bosch Lecture 15
    if mean_molecular_weight is None:
        mean_molecular_weight = mu
    halo_profile_factor = 3.0/2.0
    V_vir = Vel_Virial_analytic(M_vir_in_Msun, z, mdef=mdef)
    T_vir = halo_profile_factor* mean_molecular_weight*mp*V_vir**2 /kB/3
    return T_vir

def inversefunc_Temperature_Virial_analytic(T_vir, z, mean_molecular_weight=None, mdef='200m'):  #van den Bosch Lecture 15
    ...
    M_vir = inversefunc_Vel_Virial_analytic(V_vir, z, mdef=mdef)
    return M_vir
...
def get_gas_lognH_analytic(z, mdef='200m'):
    #default assumes 200 times the cosmological background baryon density
    #return lognH in cm^-3
    rho  = get_halo_density_analytic(z, mdef=mdef) * (Omega_b / Omega_m)
...
def get_mass_density_analytic(z, mdef='200m'):
    #default assumes 200 times the cosmological background matter density
    return get_halo_density_analytic(z, mdef=mdef)
