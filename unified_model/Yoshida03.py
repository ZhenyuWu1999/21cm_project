...
from Latif_data import Latif2019_data_copy as Latif2019_data
from Latif_data import mass_weighted_fH2
...
def plot_fH2_vs_T_from_SHMF_sampling(z):
    ...
    if converge_when_setup:
        results_file = os.path.join(output_dir, "critical_fH2_results.npz")
    else:
        results_file = os.path.join(output_dir, "critical_fH2_results_noconverge.npz") #converge_when_setup=False
    ...
    # result_halo1 = get_Latif_data()
    # Mgas_halo1 = result_halo1['Mgas_Msun']  # in Msun
    # fH2_halo1 = result_halo1['fH2_avg']
    # Mtot_halo1 = Mgas_halo1/(Omega_b/Omega_m)
    # z_halo1 = 15
    # Tvir_halo1 = lgM_to_Tvir_minihalo(np.log10(Mtot_halo1*h_Hubble), z_halo1)
    # print("Mgas_halo1 = ", Mgas_halo1)
    # print("Mtot_halo1 = ", Mtot_halo1)
    # print("fH2_halo1 = ", fH2_halo1)
    # print("Tvir_halo1 = ", Tvir_halo1)
    ...
    ax1.scatter(Tvir_halo1, fH2_halo1, color='blue', marker='o', s=80, label='Latif+ 2019 Halo1')
...
def get_Latif_profile_halo1(key, r_array):
    r_array = np.atleast_1d(r_array)  
    out = np.zeros_like(r_array, dtype=float)

    if key == "density":
        mask_inner = r_array < 1e-2
        mask_outer = ~mask_inner

        out[mask_inner] = 1e-14  # g/cm^3
        out[mask_outer] = 1e-14 * (r_array[mask_outer] / 1e-2)**(-2.5)

    elif key == "H2_mass_fraction":
        mask1 = r_array < 1e-3
        mask2 = (r_array >= 1e-3) & (r_array < 2e0)
        mask3 = (r_array >= 2e0) & (r_array < 2e1)
        mask4 = r_array >= 2e1

        out[mask1] = 3e-3

        alpha12 = (np.log10(5e-4) - np.log10(3e-3)) / (np.log10(2e0) - np.log10(1e-3))
        out[mask2] = 3e-3 * (r_array[mask2] / 1e-3)**alpha12

        alpha23 = (np.log10(1e-5) - np.log10(5e-4)) / (np.log10(2e1) - np.log10(2e0))
        out[mask3] = 5e-4 * (r_array[mask3] / 2e0)**alpha23

        out[mask4] = 5e-4 * (r_array[mask4] / 2e0)**alpha23
...
def get_Latif_data():
    ...
    r_max_pc = 1e3
    n_samples = 1000
    r_pc = np.logspace(np.log10(r_min_pc), np.log10(r_max_pc), n_samples)
    rho = get_Latif_profile_halo1("density", r_pc)
    fH2 = get_Latif_profile_halo1("H2_mass_fraction", r_pc)
    PC_TO_CM = 3.085677581e18
    MSUN_G   = 1.98847e33
    r_cm = r_pc * PC_TO_CM
    integrand = 4.0 * np.pi * (r_cm**2) * rho
    M_g = np.trapezoid(integrand, r_cm)  # g

    fH2_avg = np.trapezoid(fH2 * integrand, r_cm) / M_g
